...
def printline(liste_tuple):
    for tple in liste_tuple:
        nom,prenom,age = tple
        print(f"La personne s'appelle {prenom} {nom} et elle a {age} ans.")
    return None

def addinfo(liste_tuple):
    liste = []
    for tple in liste_tuple:
        nom,prenom,age = tple
        liste.append((nom,prenom,age,prenom[0]+nom,2023-age-1)) #(nom,prenom,age,prenom[0]+nom,2023-age-1)
    return liste

def coordtogrid(ligne,colonne,liste):
    matrix = [ [0 for j in range(colonne) ] for i in range(ligne) ]
    for (i,j) in liste: #for tple in liste tple[0] tple[1]
        matrix[j][i] = 1
    return matrix
